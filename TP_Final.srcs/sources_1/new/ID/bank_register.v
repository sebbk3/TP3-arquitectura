...
module bank_register#(
        ...
        parameter   NB_ADDR     =   5,          // Address size determinado x la cant de direcciones que podemos tener
        parameter   BANK_DEPTH  =   32          // 32 Registros diferentes
    )
    (
        input               i_clock,
        input               i_reset,
        input               i_reg_write,        // Senial de control reg_write proveniente de WB
        input [NB_ADDR-1:0] i_read_reg_a,
        input [NB_ADDR-1:0] i_read_reg_b,
        input [NB_ADDR-1:0] i_write_reg,        // Direccion del registro al que escribir 
        input [NB_DATA-1:0] i_write_data,       // Data que se va a escribir en el registro
