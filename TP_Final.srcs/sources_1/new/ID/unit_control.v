...
module unit_control#(
    ...
    (   input                       i_clock,
        ...
        output reg [NB_OPCODE-1:0]  o_alu_op,               // EX
        ...
        output reg                  o_jr_jalr,              // Hacia banco de registros
        output reg                  o_halt                  
    );
            ...
            if(!i_flush_unit_ctrl) begin
                ...
                case(i_opcode)
                    `RTYPE_OPCODE:begin
                        ...
                        o_reg_write         <= 1'b1; // escribe en banco de registro
                        o_mem_to_reg        <= 1'b0; // lee la salida ALU
                        o_jump              <= 1'b0;
                        o_byte_enable       <= 1'b0;
                        o_halfword_enable   <= 1'b0;
                        o_word_enable       <= 1'b0;
                    ...
                    `BEQ_OPCODE, `BNE_OPCODE:begin
                        ...
                        o_reg_write         <= 1'b0; // no escribe en el banco de registro
                        o_mem_to_reg        <= 1'b0; // X
                        o_jump              <= 1'b0;
                        o_byte_enable       <= 1'b0;
                        o_halfword_enable   <= 1'b0;
                    ...
                    `LB_OPCODE:begin
                        ...
                        o_reg_dest          <= 1'b0; // rt
                        o_alu_src           <= 1'b1; // immediate
                        o_mem_read          <= 1'b1; // leer mem
                        o_mem_write         <= 1'b0; // no escribir en la memoria de datos
                        o_branch            <= 1'b0; // no es branch
                        o_reg_write         <= 1'b1; // escribe en rt
                        o_mem_to_reg        <= 1'b1; // lee la salida de la memoria de datos
                        o_jump              <= 1'b0;
                        o_byte_enable       <= 1'b1;
                        o_halfword_enable   <= 1'b0;
                        o_word_enable       <= 1'b0;
                    ...
                    `LH_OPCODE:begin
                        ...
                        o_reg_dest          <= 1'b0; // rt
                        o_alu_src           <= 1'b1; // immediate
                        o_mem_read          <= 1'b1; // lee mem
                        o_mem_write         <= 1'b0; // no write mem
                        o_branch            <= 1'b0; // no es branch
                        o_reg_write         <= 1'b1; // escribe en rt
                        o_mem_to_reg        <= 1'b1; // lee salida memoria de datos
                        o_jump              <= 1'b0;
                        o_byte_enable       <= 1'b0;
                    ...
                    `LHU_OPCODE:begin
                        ...
                        o_alu_src           <= 1'b1; // immediate
                        o_mem_read          <= 1'b1; // lee mem
                        o_mem_write         <= 1'b0; // no escribe en la memoriade datos
                        o_branch            <= 1'b0; // no es branch
                        o_reg_write         <= 1'b1; // escribe en rt
                        o_mem_to_reg        <= 1'b1; // lee la salida memoria de datos
                        o_jump              <= 1'b0;
                        o_byte_enable       <= 1'b0;
                        o_halfword_enable   <= 1'b1;
                        o_word_enable       <= 1'b0;
                    ...
                    `LW_OPCODE:begin
                        ...
                        o_alu_src           <= 1'b1; // immediate
                        o_mem_read          <= 1'b1; // lee mem
                        o_mem_write         <= 1'b0; // no escribe la memoria de datos
                        o_branch            <= 1'b0; // no es branch
                        o_reg_write         <= 1'b1; // escribe en rt
                        o_mem_to_reg        <= 1'b1; // lee salida memoria de datos
                        o_jump              <= 1'b0;
                    ...
                    `LWU_OPCODE:begin
                        ...
                        o_reg_dest          <= 1'b0; // rt
                        o_alu_src           <= 1'b1; // immediate
                        o_mem_read          <= 1'b1; // lee la mememoria de datos
                        o_mem_write         <= 1'b0; // no escribe la memoria de datos
                        o_branch            <= 1'b0; // no es branch
                        o_reg_write         <= 1'b1; // escribe en rt
                        o_mem_to_reg        <= 1'b1; // lee salida la memoria de datos
                        o_jump              <= 1'b0;
                        o_byte_enable       <= 1'b0;
                        o_halfword_enable   <= 1'b0;
                        o_word_enable       <= 1'b1;
                    ...
                    `SB_OPCODE:begin
                        ...
                        o_reg_dest          <= 1'b0; // rt
                        o_alu_src           <= 1'b1; // immediate
                        o_mem_read          <= 1'b0; // no lee la mememoria de datos
                        o_mem_write         <= 1'b1; // escribe la memoria de datos
                        o_branch            <= 1'b0; // no es branch
